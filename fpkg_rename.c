...
typedef struct sfo_header_t {
  ...
  typedef struct indexTableEntry_t {
     unsigned  short keyOffset; //offset of keytable + keyOffset
     unsigned  short param_fmt; //enum (see below)
     unsigned  int paramLen;
     unsigned int paramMaxLen;
     unsigned int dataOffset; //offset of datatable + dataOffset
 } indexTableEntry_t;
...
char *title(unsigned char *p) {
	sfo_header_t *h = malloc(0x20);	
	indexTableEntry_t *e = malloc(0x10);
	int i;
	...
	memcpy(h, p, 0x20);
	for(i = 0; i < h->indexTableEntries; i++) {
		memcpy(e, p + 0x14 + (i * 0x10), 0x10);
			
		unsigned char *key;		
		key = malloc(0x16);		
		memcpy(key, p + h->keyTableOffset + e->keyOffset, 0x16);		
		if (!strncmp(key, "TITLE", strlen(key))) {
			title = malloc(e->paramLen);
			sprintf(title, "%.*s", e->paramLen, p + h->dataTableOffset + e->dataOffset);			
		} else if (!strncmp(key, "TITLE_ID", strlen(key))) {
			title_id = malloc(e->paramLen);
			sprintf(title_id, "%.*s", e->paramLen, p + h->dataTableOffset + e->dataOffset);			
		} else if (!strncmp(key, "VERSION", strlen(key))) {
			version = malloc(e->paramLen);
			sprintf(version, "%.*s", e->paramLen, p + h->dataTableOffset + e->dataOffset);			
		}		
	}	
	
	if ( title && title_id && version) {
		char *res = malloc(256);
		sprintf(res, "%s - (%s) (%s).pkg", title, title_id, version);
		return res;
	} else {
		return NULL;
	}
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < bswap_16(m_header.table_entries_num); i++) {
		...
		if ( entry_files[i].name == "param.sfo" ) {
